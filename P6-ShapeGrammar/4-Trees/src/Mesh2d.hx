...
class Mesh2d {
  ...
  **/
  ...
  public function draw(g2:Graphics, amount:Float) {
    final last = Math.min(lines.length, (amount * lines.length)).round();
    for (i in 0...last) {
      final a = lines[i].start.add(offset).mult(scale);
      final b = lines[i].end.add(offset).mult(scale);
